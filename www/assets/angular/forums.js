app.controller("forums", ['$scope', '$window', '$document', '$sce', '$http', function ($scope, $window, $document, $sce, $http) {
    ...
    $scope.thread = '';
    $scope.board = '';
    $scope.my_query = false;

    $scope.copylink = '';
    $scope.postsCount = 0;

    $scope.setThread = function () {
        $http.get('http://www.eztoeic.com/main/community', {get: 'thread', id: $scope.query, thread: $('#thread').val()});
    };

    $scope.setBoard = function () {
        $http.get('http://www.eztoeic.com/main/community', {get: 'board', id: $scope.forum, board: $('#board').val()});
    };

    $scope.deleteQuery = function (query) {
        $http.get('http://www.eztoeic.com/main/community', {get: 'deleteQuery', id: query}).
            then(function(resp) {
    ...
    $scope.deleteForum = function (forum) {
        $http.get('http://www.eztoeic.com/main/community', {get: 'deleteForum', id: forum}).
            then(function(resp) {
                $scope.formulize();
            });
    };
    ...
    $scope.deleteMe = function (id) {
        $http.get('http://www.eztoeic.com/main/community', {'get': 'deletepost', 'id': $('#pickme').attr('name')}).
            then(function(resp) {
    ...
    $scope.submit = function (id) {
        $http.get('http://www.eztoeic.com/main/postit', {
    ...
    $scope.styleBox = function (style, event, reverse) {
        ...
        if ($.contains($('#textarea').get(0),node))
        {
    ...
    $scope.forumulate = function (id) {
        $http.get('http://www.eztoeic.com/main/community?get=forum&id=' + id).
            then(function(resp) {
                $('#posts').fadeOut(500);
                $('#forums').fadeOut(500);
                $scope.data = resp.data.queries;
                $('#queries').fadeIn(500);
    ...
    $scope.edit = function (id) {
        $http.get('http://www.eztoeic.com/main/community', {'get': 'edit', 'id': id}).
            then(function(resp) {
                $('#textarea').html(resp.data.post.message);
            });
    };

    $scope.postulate = function (id, forum) {
        $scope.postsCount = 0;
        $http.get('http://www.eztoeic.com/main/community?get=post&id=' + id + '&forum=' + forum).
            then(function(resp) {
                $('#queries').fadeOut(500);
                $scope.data = resp.data.posts;
                $scope.forum = resp.data.forum;
                $scope.query = resp.data.query;
                $scope.thread = resp.data.thread;
                $scope.my_query = resp.data.my_query;
    ...
    $scope.formulize = function () {
        $('#queries').fadeOut(500);
        $http.get('http://www.eztoeic.com/main/community', {'get': 'index'}).
            then(function(resp) {
                $scope.data = resp.data.forums;
                $('#forums').fadeIn(500);
                var width = $('#frame2').width();
                var framehold = $('#frameholder');
                var height = $('#frame1').get(0).scrollHeight;
                $('#frame1').css('height', height);
                framehold.draggable({
                    containment: [-width,-height,width,0]
                });
                $('#frame1').css('top',0);
            });
    };
